...
import { ChestPrizeProgram } from "../../ChestPrizeProgram.sol";
...
type ChestPrizeProgramType is bytes32;

// equivalent to WorldResourceIdLib.encode({ typeId: RESOURCE_SYSTEM, namespace: "DUST_HACK_1", name: "ChestPrizeProgra" }))
ChestPrizeProgramType constant chestPrizeProgram = ChestPrizeProgramType.wrap(
  0x7379445553545f4841434b5f3100000043686573745072697a6550726f677261
);
...
library ChestPrizeProgramLib {
  ...
  function _msgValue(ChestPrizeProgramType self) internal view returns (uint256 __auxRet0) {
  ...
  function callFrom(ChestPrizeProgramType self, address from) internal pure returns (CallWrapper memory) {
    return CallWrapper(self.toResourceId(), from);
  }

  function callAsRoot(ChestPrizeProgramType self) internal view returns (RootCallWrapper memory) {
    return RootCallWrapper(self.toResourceId(), WorldContextConsumerLib._msgSender());
  }

  function callAsRootFrom(ChestPrizeProgramType self, address from) internal pure returns (RootCallWrapper memory) {
    return RootCallWrapper(self.toResourceId(), from);
  }

  function toResourceId(ChestPrizeProgramType self) internal pure returns (ResourceId) {
    return ResourceId.wrap(ChestPrizeProgramType.unwrap(self));
  }

  function fromResourceId(ResourceId resourceId) internal pure returns (ChestPrizeProgramType) {
    return ChestPrizeProgramType.wrap(resourceId.unwrap());
  }

  function getAddress(ChestPrizeProgramType self) internal view returns (address) {
...
using ChestPrizeProgramLib for ChestPrizeProgramType global;
using ChestPrizeProgramLib for CallWrapper global;
using ChestPrizeProgramLib for RootCallWrapper global;
